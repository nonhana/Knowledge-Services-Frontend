<template>
  <div name="firstPageMain" class="firstPageMain-wrap">
    <el-row class="header">
      <span class="title">制度地图</span>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="64 64 896 896" width="13" height="13"
          style="fill: rgba(24, 144, 255, 1);margin: 0 8px 0 0;">
          <path
        ...
        <span style="margin: 5px">更新时间：2020-04-21 10:34</span>
      ...
      <div style="width: 316px;">
        <el-row style="padding: 10px 0 0 10px;">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="24" height="24"
            ...
            <path d="M16.058 6.4l10.609 18.624h-21.333z"></path>
          </svg>
        </el-row>
        ...
        <el-row class="listWindow">
          <ul>
            <li v-for="(item, index) in nationalLaws.filter(obj => obj.type === showNationalLaws)" :key="index">
              <span>{{ item.title }}</span>
            </li>
          </ul>
        </el-row>
        <el-pagination v-model:current-page="nationalLawscurrentPage" v-model:page-size="nationalLawspageSize"
          :page-sizes="[100, 200, 300, 400]" :background="true" :pager-count="4" layout=" prev, pager, next" :total="800"
          @size-change="nationalLawsSizeChange" @current-change="nationalLawsCurrentChange" />
      </div>
      <div style="width: 316px;">
        <el-row style="padding: 10px 0 0 10px;">
        ...
        <el-menu :ellipsis="true" :default-active="'0'" class="el-menu-demo" mode="horizontal"
          @select="GovernPoliciesSelect">
          <el-menu-item v-for="(item, index) in provinceList" :index="String(index)" :key="index">{{ item
          }}</el-menu-item>
        </el-menu>
        <el-radio-group size="small" v-model="city" @change="cityChange">
          <el-radio-button v-for="(item, index) in cityList.filter(obj => obj.province === province)[0].list" :key="index"
            :label="item" />
        </el-radio-group>
        <el-row class="listWindow">
          <ul>
            <li v-for="(item, index) in GovernPolicies.filter(obj => obj.province === province && obj.city === city)"
              :key="index">
              <span>{{ item.title }}</span>
            </li>
          </ul>
        </el-row>
        <el-pagination v-model:current-page="GovernPoliciescurrentPage" v-model:page-size="GovernPoliciespageSize"
          :page-sizes="[100, 200, 300, 400]" :background="true" :pager-count="4" layout=" prev, pager, next" :total="800"
          @size-change="GovernPoliciesSizeChange" @current-change="GovernPoliciesCurrentChange" />
      </div>
      <div style="width: 316px;">
        <el-row style="padding: 10px 0 0 10px;">
        ...
        <el-menu :ellipsis="true" :default-active="'1'" class="el-menu-demo" mode="horizontal"
        ...
        <el-row class="listWindow">
          <ul>
            <li v-for="(item, index) in industryReg.filter(obj => obj.type === showIndustryReg)" :key="index">
              <span>{{ item.title }}</span>
            </li>
          </ul>
        </el-row>
        <el-pagination v-model:current-page="industryRegcurrentPage" v-model:page-size="industryRegpageSize"
          :page-sizes="[100, 200, 300, 400]" :background="true" :pager-count="4" layout=" prev, pager, next" :total="800"
          @size-change="industryRegSizeChange" @current-change="industryRegCurrentChange" />
      </div>
    </el-row>

    <el-row type="flex" style="width: 90%;align-items: center;margin: 58px 0 22px 0;">
